...
processImports([Impt = #importType{namespace = Ns} | Tail],
    ...
    case lists:member(Ns, Imported) of
        ...
        _ ->
            %% read file
            case InclFun(Ns, Impt#importType.schemaLocation, InclFiles, Dirs) of
                %% skipping import if include_fun returns nothing
                {'undefined', 'undefined'} -> processImports(Tail, Acc);
                {Xsd, Prefix1} ->
